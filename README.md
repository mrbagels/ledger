...
npx @kylebegeman/ledger init --with-docs
```
...
For accounts with two-factor authentication, npm will either complete the
browser flow or prompt for a one-time password. npm CLI publish still responds
to write-time 2FA challenges through the `--otp` option; an account passkey can
be enough for website login but may not provide a terminal OTP for `npm
publish`.

If the account uses passkeys without a TOTP code, use one of these paths:

- Add an authenticator app/TOTP method to the npm account, then run
  `npm publish --access public --otp=<code>`.
- Create a granular npm access token with publish permission and "bypass 2FA"
  enabled, then publish with `NODE_AUTH_TOKEN` set in the environment.
- Prefer npm trusted publishing from GitHub Actions once the package has a
  trusted publisher configured.

The first publish of the scoped package creates `@kylebegeman/ledger` on npm
when the account can publish under the `@kylebegeman` scope.

### Publishing To Homebrew

Homebrew needs a tap repository and a stable package URL. Publish npm first,
then create or update `mrbagels/homebrew-tap` with a `Formula/ledger.rb`
formula that installs the published npm tarball for the same version.

```bash
gh repo create mrbagels/homebrew-tap --public
brew tap-new mrbagels/tap
```
